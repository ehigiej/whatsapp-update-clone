...
let form = document.querySelector(".signup form") //Get The Form
...
form.addEventListener("submit", async(e)=>{
    e.preventDefault();
    /* Create FormData to hold signup form informations */
    let signUpForm = new FormData(form);
    let {data} = await axios.post("/signup", signUpForm);
    if(data === "success") {
        //If signup is successful, redirect to user's page 
        location.href = "/users";
    } else {
